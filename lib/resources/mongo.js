    ...
    _getDB(){
        return new Promise((resolve, reject) =>{
            ...
            client.connect(this._connectionString, 
                            ...
                            function(err, db) {
                                if (err){
                                    _log('MONGO CONNECTION ERROR:',err);
                                    reject(err);
                                } 
                                resolve(db);
                                db.on('close', function () {
                                    _log('MONGO CONNECTION event close');
                                });
                                db.on('error', function (err) {
                                    console.log('MONGO CONNECTION event error:', err);
                                });
                                db.on('disconnect', function (err) {
                                    console.log('MONGO CONNECTION event disconnect', err);
                                });
                                db.on('disconnected', function (err) {
                                    console.log('MONGO CONNECTION event disconnected', err);
                                });
                            })
        });
    }
